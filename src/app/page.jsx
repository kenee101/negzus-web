...
// Navigation Link Component with smooth transitions
    ...
    <>
      <header 
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-out ${
          isScrolled 
            ? 'bg-black/95 backdrop-blur-xl shadow-2xl shadow-emerald-500/10 border-b border-emerald-500/20' 
            : 'bg-transparent'
        }`}
      >
          ...
          <div className="flex items-center justify-between">
            ...
            >
              <div className="relative w-6 h-6">
                ...
                <span className={`absolute block w-6 h-0.5 bg-current transition-all duration-300 top-3 ${isMenuOpen ? 'opacity-0' : ''}`}></span>
...
const HeroSection = () => {
  return (
    <section id="hero" className="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      {/* Scroll Indicator */}
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 animate-bounce cursor-pointer" onClick={() => smoothScrollTo('features')}>
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            ...
            >
              <div className={`inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-r ${feature.color} mb-6 group-hover:scale-110 transition-transform duration-500`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 relative z-10">
            {steps.map((step, index) => (
              <div key={index} className="text-center group">
                ...
                <h3 className="text-2xl font-bold text-white mb-4 group-hover:text-emerald-400 transition-colors duration-300">{step.title}</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          {plans.map((plan, index) => (
            ...
            >
              ...
              <button className={`w-full py-3 px-6 rounded-2xl font-bold transition-all duration-500 ${
                plan.popular
